...
export class HelpCenterService {
  ...
  async processQuery(message: string, categoryId: string): Promise<HelpCenterResponse> {
    ...
    // 3. Let Gemini find the best match and detect safety actions
    const aiResult = await this.matchFaqWithAi(message, faqsJson);
    
    return {
      intent: aiResult.intent,
      question: aiResult.question || "No specific match found",
  ...
  private async matchFaqWithAi(message: string, faqsJson: string) {
    const { system, user } = aiPrompts.matchFaq(message, faqsJson);
